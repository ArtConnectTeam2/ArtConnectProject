...
<div class="col-xs-12 col-sm-12 col-md-10">
    <header role="work-title">
        ...
        <div style="margin-top: 20px;">
            <img src="${pageContext.request.contextPath}/resources/img/program/${program.programImg}" alt="${program.programID} 이미지" class="img-responsive" />
        </div>
    </header>
</div>
			...
			<div class="col-xs-12 col-sm-12 col-md-12" style="margin-top: 10px;">
					<section>
						<p>
							<strong>프로그램 대표 아티스트 : </strong> ${program.artist} <br />
							<strong>프로그램 전시실 : </strong> ${program.programRoom} <br />
							<strong>프로그램 전시기간 : </strong>
   							<fmt:formatDate value="${program.programStart}" pattern="yyyy년 MM월 dd일" /> -
    						<fmt:formatDate value="${program.programEnd}" pattern="yyyy년 MM월 dd일" /> <br />
							...
							<strong>프로그램 문의처 : </strong>${program.programTel}<br />
						</p>
							<strong>프로그램 예약</strong><br /> <a
								href="${pageContext.request.contextPath}/reservation/reservation/${program.galleryID}/${program.programID}">
...
<br> 
    ...
    <c:choose>
        <c:when test="${program.programID eq 1}">
            <p><strong style="font-weight: bold;">MMCA 현대차 시리즈</strong> 2023 2023.09.06 ~ 2024.02.25 <br>
            	   <strong style="font-weight: bold;">김구림</strong> 2023.08.25 ~ 2024.02.12 <br>
            	   <strong style="font-weight: bold;">올해의 작가상</strong> 2023 2023.10.20 ~ 2024.03.31 <br>
            	   <strong style="font-weight: bold;">프로젝트 해시태그</strong> 2023 2023.11.03 ~ 2024.04.07<br> 
        ...
        <c:when test="${program.programID eq 2}">
            <p><strong style="font-weight: bold;">갈라 포라스-김(2전시실)</strong><br>
			...
			갈라 포라스-김은 MUAC(멕시코시티), Kadist(파리), 아만트 재단(뉴욕), Gastworks(런던)과 CAMSTL(세인트루이스) 등에서 개인전을 개최하였으며, 휘트니 비엔날레(2019)와 우랄 산업 비엔날레(2019), 광주 비엔날레(2021), 상 파울로 비엔날레(2021), 제주 비엔날레(2022-2023), 리버풀 비엔날레(2022-2023) 등에 참여하였다. <br>
			작가는 2019년 하버드 대학교 래드 클리프 연구소의 펠로우를 하였으며, 게티 리서치 센터(2022-2022)의 아트 레지던시에 참여하였다. 또한, 그녀는 예일 미술대학 조각 학과의 시니어 크리틱으로 재직하고 있다. <br>
<br>
			<strong style="font-weight: bold;">전소정(2전시실)</strong> <br>
			전소정은 영상, 사운드, 조각, 설치, 퍼포먼스, 책 등 다양한 매체로 작업해 왔다. 
...
<br>
			<strong style="font-weight: bold;">이강승(3전시실)</strong> <br>
...
<br>
			<strong style="font-weight: bold;">권병준(4전시실)</strong> <br>
			...
			2005년부터 네덜란드에 거주하며 실험적인 전자악기 연구개발기관인 스타임(STEIM)에서 하드웨어 엔지니어로 일하다가 2011년 귀국하여 현재까지 소리와 관련한 하드웨어 연구자로 활발하게 활동하고 있으며 새로운 악기, 무대장치를 개발, 활용하여 극적 장면을 연출함으로써 음악, 연극, 미술을 아우르는 뉴미디어 퍼포먼스를 기획 연출하고 있다. <br>
			엠비소닉 기술을 활용한 입체음향이 적용된 소리기록과 전시공간 안에서의 재현 관련 기술 개발에 선두적으로 참여하고 있으며 현재 로봇을 이용한 기계적 연극을 연출하고 있다.<br>
<br>
* 권병준 ‹오묘한 진리의 숲›(2023) 은 매일 오후 2시에서 5시까지 감상하실 수 있습니다.<br>
<br>
* 4 전시실(권병준)은 매주 수, 토 오후 6-9시 사이에 작품 보수가 진행됩니다.</p>
        </c:when>
        
         <c:when test="${program.programID eq 3}">
            <p>MMCA 현대차 시리즈는 한국 현대미술의 지평을 넓히고 한국의 주요 작가들의 국제적 위상을 높이는 계기를 마련하고자 2014년부터 마련된 연례 프로그램으로, 10회째를 맞이한 2023년에는 작가 정연두가 선정되었다. <br>
            ...
            특히 2014년 이후에는 전쟁, 재난, 이주, 국가, 이데올로기와 같은 거시적 내러티브를 개인 서사 및 신화와 설화 등을 통해 재구성하고, 시와 음악, 연극의 언어를 경유하면서 다큐멘터리적 서사가 조직되는 과정에서 드러나지 않은 공백들과 다층적인 목소리에 주목하였다.<br>
<br>
...
<br>
		...
		<c:when test="${program.programID eq 4}">
            <p>리솔츠는 ‘생각하는 아름다움’의 가치가 깃든 스킨, 헤어, 바디 케어 제품을 제안합니다.<br> 
...
<br>			
			<strong style="font-weight: bold;">Show 1. 균형된 삶의 아름다움(The beauty of a balanced life)</strong><br>
...
<br>
			<strong style="font-weight: bold;">Show 2. 생각하는 아름다움(The beauty of thoughts)</strong><br>
...
<br>			
        ...
        <c:when test="${program.programID eq 5}">
            <p>대담하고 도발적인 디자인으로 널리 찬사를 받는 세계적인 디자이너 스테판 사그마이스터(Stefan Sagmeister)는 이번 전시에서 과거와 현재의 데이터를 비교해 ‘지금이 더 나은 세상이다’ 라는 긍정적인 메시지를 보여주는 100여점의 작품을 선보입니다.<br>
...
<br> 
			아래 야외작품 We’d rather be ALIVE than DEAD은 2023년 12월31일부로 운영 마감되었습니다.<br> 
...
<br> 
        ...
        <c:when test="${program.programID eq 7}">
            <p>현대의 한국화는 동양의 전통적 사유체계를 기반으로 다양한 철학적 변주를 보여주고 있습니다. <br> 
...
<br> 
        ...
        <c:when test="${program.programID eq 8}">
            <p>2022년 7월 26일부터 2023년 3월 12일까지 서울시립 북서울미술관은 관람객 참여형 어린이 전시《서도호와 아이들 : 아트랜드를 개최했습니다. <br> 
...
<br>            
        ...
        <c:when test="${program.programID eq 9}">
            <p>2023 유휴공간 프로젝트 빛나는 방, 속삭이는 거울(Glowing Chambers, Whispering Mirrors)새롭게 조성되어 시민들의 활용도가 높은 북서울미술관 2층 ‘라운지’와 3층 아트라이브러리 유휴공간에 람한 작가의 작품을 전시함으로써 서울시립미술관의 소장품 향유 기회를 확대합니다.<br>
<br>            
● 		<strong style="font-weight: bold;">유휴공간 프로젝트</strong><br>
<br>
			아파트 숲 사이 공원에 위치한 북서울미술관의 다양한 진입로 및 관내외 유휴공간을 동시대 작가들의 작품을 통해 활성화 시키는 프로젝트이다. <br>
        ...
        <c:when test="${program.programID eq 10}">
            <p>성북구립미술관은 2022년 7월 30일부터 2024년 6월 30일까지 미술관 외부에 위치한 복합문화공간 ‘거리갤러리’에서 공공미술 프로젝트로 기획된 조각가 정현의 『시간의 초상을 개최한다.<br>
<br>
